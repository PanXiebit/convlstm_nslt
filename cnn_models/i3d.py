...
class Unit3D(snt.Module):
    """Basic unit containing Conv3D + BN + non-linearity"""

    def __init__(self, output_channels, kernel_shape=(1, 1, 1), stride=(1, 1, 1), activation_fn=tf.nn.relu,
    ...
    def __call__(self, inputs, is_training):
        ...
        if self._use_batch_norm:
            ...
            net = bn(net, is_training=is_training,
                     test_local_stats=False)  # test_local_stats: if not, moving averages are used
...
class InceptionI3d(snt.Module):
    ...
    def __call__(self, inputs, is_training, dropout_keep_prob=0.5):
...
if __name__ == "__main__":
    inputs = tf.random.normal([1, 200, 224, 224, 3], dtype=tf.float32)
    out, _ = InceptionI3d(num_classes=400, final_endpoint="Logits")(inputs, is_training=True)
    print(out.shape)